...
const CaseDetailsPage = () => {
  ...
  if (isAfterCareStatus()) {
    ...
    const [schoolAddress, setSchoolAddress] = useState(edu?.school_address || '');
    const [yearCompleted, setYearCompleted] = useState(edu?.year_completed || '');
    ...
    const handleSaveSchool = async () => {
      try {
        ...
        const existing = {};
        if (Array.isArray(caseData.educationalAttainment)) {
          caseData.educationalAttainment.forEach((item) => {
        ...
        const targetKey = labelToKey(grade || gradeLevel || 'Others');
        existing[targetKey] = {
          schoolName: school,
          schoolAddress,
          year: yearCompleted
        };
        await axios.put(`${API_BASE}/cases/${caseData.id}`, { educationalAttainment: existing }, { headers });
        const updatedArray = Array.isArray(caseData.educationalAttainment) ? [...caseData.educationalAttainment] : [];
        const idx = updatedArray.findIndex((i) => String(i.level).trim() === String(grade || gradeLevel || 'Others').trim());
        const updatedItem = {
          level: grade || gradeLevel || 'Others',
          school_name: school,
          school_address: schoolAddress,
          year_completed: yearCompleted
        };
        if (idx >= 0) updatedArray[idx] = updatedItem; else updatedArray.push(updatedItem);
        setCaseData({ ...caseData, educationalAttainment: updatedArray });
        setEditMode(false);
      } catch (e) {
            ...
            <div className="mt-3">
              {!editMode ? (
                <button className="btn btn-outline-secondary btn-sm" onClick={() => setEditMode(true)}>
                  <i className="fas fa-edit me-1"></i>Edit School
                </button>
              ) : (
                <div className="row g-2 align-items-end">
                  ...
                  <div className="col-md-9 d-flex gap-2 justify-content-end">
                    <button className="btn btn-outline-secondary btn-sm" onClick={() => setEditMode(false)} disabled={saving}>Cancel</button>
                    <button className="btn btn-primary btn-sm" onClick={handleSaveSchool} disabled={saving || !school}>Save</button>
                  </div>
                </div>
              )}
            </div>
